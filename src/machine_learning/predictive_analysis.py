...
def plot_predictions_probabilities(pred_proba, pred_class):
    ...
    prob_per_class = pd.DataFrame(
        data=[0, 0],
        index={'Free from': 0, 'infected with': 1}.keys(),
        columns=['Probability']
    )
...
def load_model_and_predict(my_image, version):
    ...
    model = load_model(f"outputs/{version}/leaf_identifier_model.h5")

    pred_proba = model.predict(my_image)[0, 0]

    target_map = {v: k for k, v in {'Free from': 0, 'infected with': 1}.items()}
